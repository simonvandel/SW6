...
\begin{enumerate}[resume]
    \item \textbf{Location data must be acquired using the aSTEP core.} Our application must use the aSTEP core functionalities to acquire the location data used to visualise the behaviour of the crowds. This requirement is given a high priority.
    \item \textbf{The application must be maintainable.} During the development of the application, maintainability should be prioritised. This facilitates development of the application in subsequent sprints and on future semesters. This requirement is given a medium priority, as it is does not directly impact the usability of the application.
    \item \textbf{The application must be extensible.} Extensibility should also be a priority during the development, ensuring that features can be added in later development stages. This also facilitates future development on the application. This requirement is also given a medium priority.
\end{enumerate}
