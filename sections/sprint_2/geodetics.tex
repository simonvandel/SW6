...
Whenever distances are calculated the required properties should be considered, and the appropriate method chosen. For this project we will be using Haversine since the limited distances will minimise the inaccuracies of the formula, and that the better performance is helpful during the calculation of the crowd factors described in \cref{sec:related_work}.\jenote{maybe we should say how inaccurate it is and why this is good enough for us?}
...
\jenote{maybe argue that the amount of skewing that would occur on smukfest and say that that is acceptable again?}
