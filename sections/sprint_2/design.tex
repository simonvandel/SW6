...
We use the model-view-controller (MVC)\kanote{insert reference}\cite{} architectural pattern for the web application, as this pattern supports low coupling and high cohesion. The aSTEP project follows a client-server architecture, where the core represents the server, and the applications are the different clients. As our application also follows a client-server architecture, a HotMap server will act as a middle-end between the aSTEP core and the actual users. The architecture is shown in \cref{fig:architecture_diagram}.
