...
Colour represents the value of the displayed crowd factor, e.g. the value for the density overlay could be 2 people per square metre. The colour scheme ranges from low values as blue to high values as red. What denotes a high value depends on the crowd factor being displayed. \Cref{tab:color_scale} explains the colour scale. As different people associate different values to colours, it is difficult to find a universal colour scheme that is perfect for everyone. This colour scheme is arbitrarily chosen to range from blue to red, but this should be reconsidered according to each specific users needs, before being put into actual use.
...
\definecolor{highest}{HTML}{FF0000}
    \centering
    \sinote{disse tal skal passe med de tal i \cref{subsec:levelsOfHazard}}
