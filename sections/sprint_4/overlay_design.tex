...
The opacity in this overlay differ from the rest. Instead of letting the opacity scale on density, it depends on the speed of the crowd. Density is still important when considering the danger level of the crowd velocity, but since the velocity overlay can easily be combined with the density overlay, opacity can be used in a different way. 


It does not make sense to display the direction of a crowd that moves at a slow pace, because there is no dominating direction. Additionally, as the surface area of the arrow is small, it can be hard to differentiate different opacity values. We therefore use a binary opacity; Either an arrow is shown, or it is not. 
