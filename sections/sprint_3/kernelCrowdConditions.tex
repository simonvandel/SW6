...
When calculating the crowd densities, using kernel density estimation, we insert the \enquote{intensity} of a person into the formula. The intensity, in this case, is be the amount which a given person contributes to the density, which is his physical size. However, since we have no way of deducting a persons size based on his position data, we assume that every person is the same size, which would be \enquote{1 person}. This gives us the formula:
